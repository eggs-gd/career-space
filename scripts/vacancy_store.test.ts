 ...
 *   confirms the new code reads a record shaped exactly like what the Python original wrote
 *   (microsecond-precision timestamp, same field set) without throwing. Runs on every clone,
 *   including a clean OSS one with no `data/` at all.
...
test("reads a synthetic Python-shaped record (microsecond timestamp, '+00:00' offset) without throwing", () => {
  ...
  // needs to survive against every record the Python original ever wrote.
...
test("listVacancies reads real on-disk data without throwing, when present", { skip: !fs.existsSync(DATA_DIR) }, () => {
  ...
  for (const v of all) {
    ...
    // Every real record here was written by the Python original -- same blanket check as the
    // synthetic fixture above, just against whatever's actually on this machine.
