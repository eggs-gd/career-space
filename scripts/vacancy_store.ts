...
export interface ScoutOutcomeCandidate {
  // Omit both for a manually-obtained posting (a URL `resolve_vacancy_url` returned
  // `matched: false` for, then read by hand) -- `recordScoutOutcome` computes them via
  // `posting_ids.manualIds`, the same path `upsertVacancy` uses for a hand-pasted posting.
  // Scout-fetched candidates always carry real ids and pass them through.
  posting_id?: string;
  content_id?: string;
  company: string;
  title: string;
...
export function recordScoutOutcome(opts: RecordScoutOutcomeOptions): RecordedScoutOutcome {
  ...
  const [postingId, contentId] =
    candidate.posting_id && candidate.content_id
      ? [candidate.posting_id, candidate.content_id]
      : postingIds.manualIds(candidate.company, candidate.title, candidate.job_post_text, candidate.url ?? "");
...
/** Re-renders the board after a CLI state change, matching the `vacancy_*` MCP tools. `require`,
 * not a top-level import: `render_board.ts` depends on this file (via `listVacancies`), so a
 * top-level import back would cycle. Safe lazily -- only ever runs from `cli()` below. */
...
function cli(): void {
  ...
  if (command === "mark-seen") {
    ...
    if ((result as RecordedScoutOutcome[]).some((r) => r.outcome === "matched")) renderBoardsFromCli();
