...
import { pythonRound, computeScore } from "./score_fit";

test("pythonRound uses banker's rounding at exact .5 boundaries, unlike Math.round", () => {
  assert.equal(pythonRound(2.5), 2, "2.5 rounds to even (2), not up to 3");
  assert.equal(pythonRound(3.5), 4, "3.5 rounds to even (4)");
  assert.equal(pythonRound(0.5), 0);
  assert.equal(pythonRound(1.5), 2);
  assert.equal(pythonRound(-0.5), 0, "floor(-0.5) = -1, diff = 0.5, -1 is odd -> rounds to 0");
...
// Cross-checked against the Python original (scripts/score_fit.py's compute_score/render) for
// several representative inputs -- see the manual fixture comparison run during the port; every
// case produced a byte-identical score and rendered Markdown output.
